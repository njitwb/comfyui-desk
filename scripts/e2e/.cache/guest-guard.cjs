...
// src/main/guest-guard.ts
var guest_guard_exports = {};
__export(guest_guard_exports, {
  installGuestGuards: () => installGuestGuards,
  isModelUrlStr: () => isModelUrlStr
});
module.exports = __toCommonJS(guest_guard_exports);
...
// src/main/guest-guard.ts
var MODEL_URL_RE = /[\w.-]+\.(?:safetensors|ckpt|pt|pth|bin|gguf|onnx|sft)([?#].*)?$/i;
function isModelUrlStr(url) {
  return /^https?:\/\//i.test(url) && MODEL_URL_RE.test(url);
}
function allowLocalNav(url) {
  return /^https?:\/\/(?:127\.0\.0\.1|localhost|\[::1\])(?::\d+)?(?:[/?#]|$)/i.test(url);
}
function installGuestGuards(deps) {
  import_electron.app.on("web-contents-created", (_e, contents) => {
